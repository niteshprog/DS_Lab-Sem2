...
void delete()
{
    ...
    scanf("%d", &id); 
    //get-the-key-for-the-id
    i = getkey(id); 
    //declare-few-pointers-to-visit-each-element-of-LL
    ...
    if(temp != NULL)
    {
        ...
        //else-traverse-to-the-element-and-delete-that-element
        else
        {
            while(temp->next)
            {
...
int search()
{
    //declare-variables
	int i, id, c; 
	//get-the-SAP-ID-to-delete
    printf("\nEnter the SAP ID to search: "); 
    scanf("%d", &id); 
    //get-the-key-for-entered-key
    i = getkey(id); 
    //point-to-the-index = i
    struct body *temp = table[i];
    //traverse-through-the-LL-at-index = i-and-check-if-value-is-present-or-not
    while(temp)
    {
        if(temp->data == id)
            c = 1;
    ...
    //if-present-print-family-number-else-notify-the-user
    if(c == 1)
    ...
    while(i = 1)
    {
		...
		switch(job)
		{
		    ...
		    case 2:
				...
				break;
		    case 3:
		    	//invoke-delete-function-to-delete-items
				delete(); 
				break; 
			case 4: 
				//invoke-search-function-to-search
			    search(); 
			    break; 
			 case 5:
			 	//exiting-the-program 
